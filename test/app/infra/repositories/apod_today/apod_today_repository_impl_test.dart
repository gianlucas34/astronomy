...
import 'apod_today_repository_impl_test.mocks.dart';

@GenerateNiceMocks([MockSpec<ApodTodayDatasource>(), MockSpec<NetworkInfo>()])
void main() {
  final datasource = MockApodTodayDatasource();
  final networkInfo = MockNetworkInfo();
  final repository = ApodTodayRepositoryImpl(datasource, networkInfo);
  ...
  test(
    'Should return a ApodModel if user network is connected',
    () async {
      when(networkInfo.isConnected).thenAnswer((_) async => true);
      when(datasource.fetchApodToday()).thenAnswer((_) async => mockedResult);
  ...
  test(
      ...
      () async {
    when(networkInfo.isConnected).thenAnswer((_) async => true);
    when(datasource.fetchApodToday()).thenThrow(const DatasourceError());
  ...
  test(
    ...
    () async {
      when(networkInfo.isConnected).thenAnswer((_) async => true);
      when(datasource.fetchApodToday()).thenThrow(Exception());
  ...
  test(
    ...
    () async {
      ...
      verifyNever(datasource.fetchApodToday());
